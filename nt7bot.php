...
$tbbots = $KKYKKN;
$Dev = array($Shtrak_id);
$eri = array("1405398498", "5372193406" );
$DevUser = "@$buy"; // 
$Devch = "@$buy"; //  
$Userbot = "$iidd"; // 
// 2 end
...
case 8: $nmonth="اغسطس";
break;
case 9: $nmonth="سبتمبر";
break;
case 10: $nmonth="اكتوبر";
break;
case 11: $nmonth="نوفمبر";
break;
case 12: $nmonth="ديسمبر";
break;
} 
...
'reply_markup'=>json_encode([
'inline_keyboard'=>[
    [['text'=>"اضافه البوت لمجموعتك ➕",'url'=>"https://t.me/nt7Bot?اشترلك الاجياريgroup=Commands&admin=delete_messages+apin_messages+invite_users"]],
...
[['text'=>"- حالة الجروب ؛ ⁉",'callback_data'=>"grup"]],
...
"reply_markup"=>json_encode([
"inline_keyboard"=>[
[['text'=>"- حالة الجروب ؛ ⁉ 2",'callback_data'=>"grupw"]],
...
- يمكنك وضع بعض الاضافات الى كليشه الاشترلك الاجياري من خلال استخدام الهشتاجات التاليه :
...
"chat_id"=>$chat_id,
"text"=>"
- تم تعيين كليشة الاشتراك الاجباري بنجاح ؛✅
...
if($text == "المشتركين" and $chat_id == $SAEEDFiles){
  bot('sendMessage',[
    'chat_id'=>$chat_id,
    'text'=>"⌔ عدد مشتركين بوتك هو { $SAEED } مشترك"
    ]);
}
...
if($text == "نشر" and $chat_id == $SAEEDFiles){
 file_put_contents("T4TTTT.txt", "SAEED");
  bot('sendMessage',[
    'chat_id'=>$chat_id,
    'text'=>"⌔ ارسل رسالتك الان وسيتم ارسالها الى { $SAEED } مشترك"
    ]);
}
...
if($update and $SAEE9D == "26000"){
for ($i=0; $i < count($SA3ED); $i++) { 
    bot('sendMessage',[
      'chat_id'=>$SA3ED[$i],
      'text'=>"⌔ تنبيه مـفرح
تفآعل آلبوت وصـل آلي *$260k* رسائل ✨ 
هذا آلخبر يفرحنآ جدآ شـگرآ لآسـتخدآمـگم آلبوت ❤
",
'parse_mode'=>"MarkDown",
'disable_web_page_preview' =>true,
    ]);
